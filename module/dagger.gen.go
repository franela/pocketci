...
func (r *Go) WithSource(src *Directory) *GoWithSource {
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Webhook":
		...
		case "Proxy":
			...
			return (*Webhook).Proxy(&parent, ctx), nil
		default:
	...
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("Webhook").
					WithFunction(
						dag.Function("Proxy",
							dag.TypeDef().WithObject("Service")).
							WithDescription("First thing first: figure out how to do a reverse proxy that does stuff before hand\n"))), nil
	default:
